...
export const sendLLMMessage = async ({
	...
	const onError: OnError = ({ message: errorMessage, fullError }) => {
		...
		if (errorMessage === 'TypeError: fetch failed')
			errorMessage = `Failed to fetch from ${displayInfoOfProviderName(providerName).title}. This likely means you specified the wrong endpoint in Edlide's Settings, or your local model provider like Ollama is powered off.`
	...
	try {
		...
		if (messagesType === 'chatMessages') {
			await sendChat({ messages: messages_, onText, onFinalMessage, onError, settingsOfProvider, modelSelectionOptions, overridesOfModel, modelName, _setAborter, providerName, separateSystemMessage, chatMode, mcpTools, supabaseAccessToken })
			return
		}
