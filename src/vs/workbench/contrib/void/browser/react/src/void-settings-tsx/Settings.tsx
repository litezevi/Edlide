...
// Project Rules components
const ProjectRuleItem = ({ fileName, onEdit, onDelete, onRename }: {
	fileName: string,
	onEdit: () => void,
	onDelete: () => void,
	onRename?: boolean
...
const ProjectRulesSection = () => {
	...
	// Setup file system polling
	useEffect(() => {
		// Initial load
		loadVoidRulesFiles();

		// Setup polling for file system changes
		const pollInterval = setInterval(loadVoidRulesFiles, 2000); // Poll every 2 seconds
		...
		try {
			...
			for (const folder of workspaceFolders) {
				...
				await commandService.executeCommand('_workbench.open', fileUri, [
					...
					{ pinned: true }  // Makes tab pinned (non-preview) - opens in new tab in same column
				]);
				break;
			}
		} catch (e) {
	...
	const handleDeleteFile = async (fileName: string) => {
		try {
			...
			await loadVoidRulesFiles(); // Refresh list
		} catch (e) {
	...
	const handleCreateFile = async () => {
		...
		try {
			...
			const fileContent = `# ${fileName}\n\n# Add your project-specific rules here\n# These will be included in all AI requests\n`;

			// Write the file
			await fileService.writeFile(fileUri, VSBuffer.fromString(fileContent));

			// Open the file in editor (new tab in same column, non-preview)
			const commandService = accessor.get('ICommandService');
			await commandService.executeCommand('_workbench.open', fileUri, [
				undefined,       // No column specified (use current column)
				{ pinned: true }  // Makes tab pinned (non-preview) - opens in new tab in same column
	...
	return (
		<div className='mt-6'>
			...
			<div className='flex items-center justify-between mb-3'>
				<h3 className='text-lg font-medium text-void-fg-1'>Project Rules</h3>
				<button
					onClick={() => setIsCreatingNew(true)}
					className='text-void-fg-3 hover:text-void-fg-1 p-1'
			...
			<div className='space-y-1'>
				{voidRulesFiles.map(fileName => (
					<ProjectRuleItem
						key={fileName}
						fileName={fileName}
						onEdit={() => handleEditFile(fileName)}
...
export const SettingsForProvider = ({ providerName, showProviderTitle, showProviderSuggestions }: { providerName: ProviderName, showProviderTitle: boolean, showProviderSuggestions: boolean }) => {
	...
	return <div>
		...
		<div className='px-0'>
			{/* settings besides models (e.g. api key) - only show if not hidden */}
			{!shouldHideSettings && settingNames.map((settingName, i) => {
...
export const OllamaSetupInstructions = ({ sayWeAutoDetect }: { sayWeAutoDetect?: boolean }) => {
	return <div className='prose-p:my-0 prose-ol:list-decimal prose-p:py-0 prose-ol:my-0 prose-ol:py-0 prose-span:my-0 prose-span:py-0 text-void-fg-3 text-sm list-decimal select-text'>
		...
		{sayWeAutoDetect && <div className=' pl-6'><ChatMarkdownRender string={`Edlide automatically detects locally running models and enables them.`} chatMessageLocation={undefined} /></div>}
	</div>
}
